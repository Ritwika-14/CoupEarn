...
import { auth, googleProvider ,db, storage,useAuth,upload} from "./config/firebase";
import {
...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div className="buy" id="buy" >
        ...
        <div class="box">
            ...
            <div class="box-head">
                <img  className="buy-image"src="https://static.vecteezy.com/system/resources/previews/019/632/927/original/3d-minimal-discount-coupon-special-offer-promotion-flash-sale-icon-3d-illustration-free-png.png" alt=""/>
                <span class="menu-category">{coupon.title}</span>
                <h3>{coupon.description}</h3>
                <div class="price">Exp Dt: <span>{coupon.date}</span></div>
            </div>
    ...
    {reviewList.map((reviews) => (
        <div class="box1">
            <img src="https://static.vecteezy.com/system/resources/previews/004/235/343/non_2x/five-star-top-review-logo-modern-chat-icon-talk-quotes-in-trendy-linear-line-style-dark-background-free-vector.jpg" alt="quote"/>
            <p>{reviews.title}</p>
        ...
        <button className="btn1" onClick={onSubmitReview}> Submit</button>
    ...
    <div>
      ...
      <a href="https://coupearn.netlify.app/" class="back">Go Back</a>
       ...
       <button disabled={loading||!photo} className="btn1" onClick={handleClick}>Upload</button>
       <div className="flex">
       <button className="btn1" onClick={signIn}> Sign In</button>
       <button  className="btn1" onClick={signInWithGoogle}> Sign In With Google</button>
       </div>
